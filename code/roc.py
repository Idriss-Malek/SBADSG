...
def plot_anograph(algorithm, dataset_name):
    ...
    for x in param:
        data = pd.read_csv(f'../results/scores_{algorithm}_{dataset_name}_{x[0]}_{x[1]}.txt', header=None, names=['Score', 'Label'], sep=',')
...
def plot_anoedge(algorithm):
    ...
    for dataset_name in datasets:
        data = pd.read_csv(f'../results/scores_{algorithm}_{dataset_name}.txt', header=None, names=['Score', 'Label'], sep=',')
        fpr, tpr, thresholds = metrics.roc_curve(data.Label, data.Score)
        roc_auc = metrics.auc(fpr, tpr)
